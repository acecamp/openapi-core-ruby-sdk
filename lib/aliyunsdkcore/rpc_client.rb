  ...
  class RPCClient
    ...
    def request(action:, params: {}, opts: {})
      ...
      response      = connection.send(method.downcase, uri) do |request|
        ...
        request.options.timeout      = opts[:timeout]      if opts[:timeout]
        request.options.open_timeout = opts[:open_timeout] if opts[:open_timeout]
        request.options.read_timeout = opts[:read_timeout] if opts[:read_timeout]
        request.options.write_timeout = opts[:write_timeout] if opts[:write_timeout]
        if opts[:method] == 'POST'
